...
filterNameInput.addEventListener('keyup', e => {
    ...
    Object.entries(getCookies()).forEach(([name, value]) => {
        if (isMatchChunks(isMatch, e.target.value, name, value)) {
            fragment.appendChild(createRow(name, value));
        }
        ...
        if (filterNameInput.value) {
            if (cookies.hasOwnProperty(name) && !isMatch(value, filterNameInput.value)) {
                ...
                for (const child of listTable.children) {
                    if (child.cells[0].innerHTML === name) {
                        child.remove();
                    }
                }
