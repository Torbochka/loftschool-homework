...
function collectDOMStat(root) {
    ...
    let classes = (node) => {
        node.classList.forEach(k => {
            (k in stat.classes) ? stat.classes[k]++ : stat.classes[k] = 1;
        });
    };
    ...
    let tags = (node) => {
        ...
        (k in stat.tags) ? stat.tags[k]++ : stat.tags[k] = 1;
    };
...
function observeChildNodes(where, fn) {
    let observer = new MutationObserver(info => {
        info.forEach(info => {
            if (info.type === 'childList') {

                if (Object.keys(info.removedNodes).length) {
                    fn({ type: 'remove', nodes: [...info.removedNodes] });
                } else {
                    fn({ type: 'insert', nodes: [...info.addedNodes] });
                }
            }
        });
    });
